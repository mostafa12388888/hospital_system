                        ...
                        <table id="example" class="table key-buttons text-md-nowrap">
                          ...
                          @foreach($doctors as $doctor)
                              <tr>
                                  ...
                                  <td>
                                    @if($doctor->image)
                                    <img src="{{asset('Dashbord/img/docutors/'.$doctor->image->fileame )}}" height=50px width="50px">
                                    @else
                                    <img src="{{asset('Dashbord/docutor.jpg' )}}" height=40px width="40px">
                                    @endif
                                  </td>
                                  <td>{{ $doctor->name }}</td>
                                  <td>{{ $doctor->email }}</td>
...
{{$appointments->translations[0]->name}}
                                    @endforeach
                                  </td>
                                  <td>{{ $doctor->price}}</td>
                                  <td>
                              ...
                              @include('Dashbord.Doctors.delete')
                              @include('Dashbord.Doctors.delete_select')
                              @include('Dashbord.Doctors.update_password')
                                @include('Dashbord.Doctors.update_status')
                          @endforeach
                            </tbody>
                        </table>
                    </div>
        ...
        $(function(){
            ...
            $("#btn_delete_all").on('click',function(){
                ...
                $('#example input[name="delete_select"]:checked').each(function(){
                    Select_value.push(this.value)
                })
                if(Select_value.length){
$('#delete_select').modal('show');
$('input[id="delete_select_id"]').val(Select_value);
                }
            })
        })
    </script>
