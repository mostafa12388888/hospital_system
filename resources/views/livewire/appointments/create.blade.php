<div>
    ...
    @if($message)
        <script>
            alert('الدكتور انتهت مواعيده لهذا اليوم برجاء الحجز في يوم اخر')
            location.reload()
        </script>
    @endif
...
@endif
                    @foreach($doctors as $doctor)
                        ...
                        <option value="{{$doctor->id}}">{{$doctor->translations[0]->name}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-lg-6 col-md-6 col-sm-12 form-group">
                ...
                <select class="form-select" name="section" wire:model="section" id="exampleFormControlSelect1">
                    ...
                    @foreach($sections as $section)
                        <option value="{{$section->id}}">{{$section->translations[0]->name}}</option>
                    @endforeach
